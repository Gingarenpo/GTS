...
public class MQOFace implements Serializable, Cloneable {
	...
	private MQOVertex[] v; // 頂点を格納（固定なのでプリミティブ配列で）
	private int[] vnum; // 頂点番号
	...
	public MQOFace(MQOObject mqo, String vnum, String uvnum) {
		...
		this.vnum = new int[vs.length];
		for (int i = 0; i < vs.length; i++) {
			this.vnum[i] = Integer.parseInt(vs[i]);
	...
	public MQOFace clone() {
		...
		clone.vnum = new int[vnum.length];
	...
	public void setUv(ArrayList<double[]> uv) {
		this.uv = uv;
	}

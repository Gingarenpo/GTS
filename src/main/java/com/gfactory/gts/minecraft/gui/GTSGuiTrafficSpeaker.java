...
public class GTSGuiTrafficSpeaker extends GTSGuiModelChoose<GTSTileEntityTrafficSpeaker> {
    ...
    public void initGui() {
        ...
        this.channelName.setText(((GTSTileEntityTrafficSpeaker) tileEntity).getChannel());
    ...
    @Override
    public TreeMap<String, GTSTrafficSpeakerConfig> getModelElements() {
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == 1) {
            // チャンネル名を更新する
            GTSTileEntityTrafficSpeaker te = (GTSTileEntityTrafficSpeaker) tileEntity;
            te.setChannel(this.channelName.getText());
            GTS.NETWORK.sendToServer(new GTSPacketTileEntity<>(te.writeToNBT(new NBTTagCompound()), te.getPos(), GTSTileEntityTrafficSpeaker.class));
            te.markDirty();
